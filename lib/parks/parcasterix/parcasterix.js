...
export class ParcAsterix extends Destination {
  ...
  async _loadPOIDatabase(entry) {
    ...
    const stringHoursToNumbers = (str) => {
      const match = str.match(/(\d+):(\d+) [ap]\.m/);
      if (match) {
        // if we're in pm, add 12 hours
        if (str.indexOf('p.m') >= 0 && match[1] !== '12') {
    ...
    const hoursMap = labels.reduce((acc, x) => {
      ...
      // look for time ranges (there may be multiple)
      //  accept either "-" or "to" as a separator
      const matches = x.value.match(/(\d+:\d+ [ap]\.m)\.? (?:-|to) (\d+:\d+ [ap]\.m)\.?/g);
      if (matches) {
   ...
   */
  async _fetchPOIData() {
    // cache the whole thing for 1 hour
