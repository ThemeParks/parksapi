  ...
  async buildEntityLiveData() {
    ...
    return liveData.map((data) => {
      ...
      if (!foundClosedText) {
        ...
        } else if (data?.infos?.textCard && data.infos.textCard.indexOf(" minutes wait") > 0) {
          // extract wait time from textCard
          const waitTimeMatch = data.infos.textCard.match(/(\d+) minutes wait/);
          if (waitTimeMatch) {
            liveDataObj.queue = {
              [queueType.standBy]: {
