...
export class Futuroscope extends Destination {
  ...
  async _getAPIToken() {
    ...
    // get the current app version from ThemeParks.wiki API
    const appVersion = await this.getAndroidAPPVersion('fr.comint.futuroscopev2', '4.1.10');
  ...
  async buildEntityLiveData() {
    ...
    return liveData.map((data) => {
      ...
      //  0 = no data, 1 = open (facilities), 2 = opening later, 3 = open with wait, 4 = show, 6 = closed
      if (data.status === 3) {
        liveDataObj.status = statusType.operating;

        // parse wait time from textCard (e.g. "15 minutes wait")
        if (data?.infos?.textCard) {
          const waitTimeMatch = data.infos.textCard.match(/(\d+)\s*min/i);
          if (waitTimeMatch) {
            liveDataObj.queue = {
              [queueType.standBy]: {
        ...
        // show with next session time
        liveDataObj.status = statusType.operating;
      } else if (data.status === 2) {
        // not yet open today (e.g. "14H00 : Opening")
        liveDataObj.status = statusType.closed;
      } else if (data.status === 6) {
        liveDataObj.status = statusType.closed;
      } else if (data.status === 1) {
        // open facility (shops, WCs) — mark as operating
        liveDataObj.status = statusType.operating;
      } else {
        // status 0 or unknown
        liveDataObj.status = statusType.closed;
