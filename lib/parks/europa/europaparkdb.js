...
import {tagType, attractionType} from '../parkTypes.js';
...
export class DatabaseEuropaPark extends Database {
  ...
  async getConfig() {
    return await this.cache.wrap('auth', async () => {
      ...
      return {
        username: decrypt(resp.body.entries.v3_live_exozet_api_username),
        password: decrypt(resp.body.entries.v3_live_exozet_api_password),
      };
  ...
  async getToken() {
    return await this.cache.wrap('token', async () => {
      ...
      const resp = await this.http(
          'POST',
          `${this.config.apiBase}/api/v1/login_check`,
          auth,
          {
            json: true,
            login: true,
    ...
    // API doesn't give us an expirey time, app keeps making requests until we get a 401 back
  }
