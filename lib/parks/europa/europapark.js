...
import {statusType, queueType, scheduleType} from '../parkTypes.js';
...
export class EuropaParkBase extends Park {
  ...
  async _init() {
    ...
    const db = EuropaParkDB.get();
    const attractions = await db.getEntities({
      // filter entities by our park ID
      '_src.scopes': {
        $elemMatch: this.config.parkId,
      },
    });
  ...
  async _update() {
    const db = EuropaParkDB.get();
    const waits = await db.getWaitingTimes();
    ...
    await Promise.allSettled(waits.waitingtimes.map(async (wait) => {
      ...
      switch (wait.time) {
        ...
        case 91:
          ...
          // TODO - how do we store return time queues?
          /* await this.updateAttractionQueue(attr.code, {
              nextAvailableStart: '',
              nextAvailableEnd: '',
      ...
      if (queue == queueType.standBy) {
        ...
        // TODO - handle return time queues
      }
    }));
  }
  ...
  async _getOperatingHoursForDate(date) {
    ...
    const cal = await db.getCalendar();
