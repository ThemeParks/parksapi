...
function extractName(name) {
  if (typeof name === 'object') {
    ...
    if (langIdx > -1) {
      return name[langs[langIdx]].trim();
    } else {
      // otherwise just pick the first one
      return Object.values(name)[0].trim();
    }
  }
...
export class AttractionsIO extends Destination {
  ...
  async buildAttractionEntities() {
    return this._buildEntitiesFromCategories([
      'Attractions',
      'Rides',
      'Water Rides',
      'Thrill Rides',
  ...
  async buildEntityLiveData() {
    ...
    return validEnts.map((x) => {
      ...
      // mark closed if IsOpen is false
      if (x.IsOpen !== undefined) {
        data.status = (x.IsOpen) ? data.status : statusType.closed;
      }
  ...
  async buildEntityScheduleData() {
    ...
    if (!scheduleData || (!scheduleData.Locations && !scheduleData.locations) || (!scheduleData.Locations?.length && !scheduleData.locations?.length)) {
      return [
        {
          _id: this.config.parkId,
          schedule: [],
    ...
    const locations = scheduleData.Locations ? scheduleData.Locations : scheduleData.locations;

    // assume 1 park per destination
    let days = locations[0].days;
    if (days.length === 0) {
      // find location with most days
      let maxDays = 0;
      locations.forEach((loc) => {
        if (loc.days.length > maxDays) {
          maxDays = loc.days.length;
          days = loc.days;
        }
      });
    }
...
export class HeidePark extends AttractionsIO {
  constructor(config = {}) {
    ...
    config.initialDataVersion = config.initialDataVersion || '2025-09-23T10:30:15Z';

    config.appBuild = config.appBuild || 302101;
    config.appVersion = config.appVersion || '4.2.6';

    super(config);
  }

  // override schedule fetching to use custom URL
  async buildEntityScheduleData() {
    // https://api.attractions.io/v2/resort-opening-times?endDate=2026-01-31&startDate=2025-10-29
    const startDate = moment().tz(this.config.timezone).startOf('day').format('YYYY-MM-DD');
    const endDate = moment().tz(this.config.timezone).add(3, 'months').endOf('day').format('YYYY-MM-DD');

    const url = `https://api.attractions.io/v2/resort-opening-times?startDate=${startDate}&endDate=${endDate}`;

    const scheduleData = await this.http('GET', url);

    if (!scheduleData || !scheduleData.body || !scheduleData.body.openingTimes || !Array.isArray(scheduleData.body.openingTimes)) {
      return [
        {
          _id: this.config.parkId,
          schedule: [],
    ...
    const schedule = scheduleData.body.openingTimes.map((x) => {
      const date = moment(x.date, 'YYYY-MM-DD').tz(this.config.timezone, true);

      if (!x.openingTimes || !x.openingTimes.open || !x.openingTimes.close) {
        return null;
      }
     
      return {
        date: date.format('YYYY-MM-DD'),
        type: (x.status === 'open') ? scheduleType.operating : scheduleType.closed,
        openingTime: date.clone().hour(Number(x.openingTimes.open.split(':')[0])).minute(Number(x.openingTimes.open.split(':')[1])).second(0).millisecond(0).format(),
        closingTime: date.clone().hour(Number(x.openingTimes.close.split(':')[0])).minute(Number(x.openingTimes.close.split(':')[1])).second(0).millisecond(0).format(),
      };
    }).filter((x) => !!x);
...
export class Knoebels extends AttractionsIO {
  ...
  // TODO - schedule is missing API
  //  https://knoebels.com/wp-admin/admin-ajax.php
  //  They are using a WordPress plugin for some reason
  async buildEntityScheduleData() {
    return [];
  }
}

export class LegolandJapan extends AttractionsIO {
  constructor(config = {}) {
    config.timezone = config.timezone || 'Asia/Tokyo';
    config.destinationId = config.destinationId || 'legolandjapanresort';
    config.parkId = config.parkId || 'legolandjapan';
    config.initialDataVersion = config.initialDataVersion || '2025-08-21T05:37:16Z';

    config.appBuild = config.appBuild || 186;
    config.appVersion = config.appVersion || '1.4.24';

    super(config);
  }
}

export class DjursSommerland extends AttractionsIO {
  constructor(config = {}) {
    config.timezone = config.timezone || 'Europe/Copenhagen';
    config.destinationId = config.destinationId || 'djurs-sommerland-destination';
    config.parkId = config.parkId || 'djurs-sommerland';
    config.initialDataVersion = config.initialDataVersion || '2025-08-22T09:45:54Z';

    config.appBuild = config.appBuild || 169;
    config.appVersion = config.appVersion || '2.5.1';
  ...
  async _fetchCalendarHTML() {
    ...
    const scheduleData = await this.http('GET', 'https://djurssommerland.dk/en/plan-your-trip/opening-hours/');
    return scheduleData.body;
  }

  // Custom HTML parsing for Djurs Sommerland calendar
  async buildEntityScheduleData() {
    const html = await this._fetchCalendarHTML();
    const $ = cheerio.load(html);

    // Extract calendar data from the body data-model attribute
    const $body = $('body');
    const dataModelAttr = $body.attr('data-model');
    ...
    let calendarData;
    try {
      calendarData = JSON.parse(dataModelAttr);
    } catch (err) {
      console.error('Failed to parse calendar data:', err);
      return [
        {
          _id: this.config.parkId,
          schedule: [],
    ...
    // Process each park event
    calendarData.parkEvents.forEach(event => {
      const {type, start, end, days} = event;

      // Skip events without proper time format
      if (!start || !end || !days || !days.ranges) return;

      // Parse opening hours (format: "10:00")
      const startMatch = start.match(/(\d{1,2}):(\d{2})/);
      const endMatch = end.match(/(\d{1,2}):(\d{2})/);

      if (!startMatch || !endMatch) return;

      const openHour = parseInt(startMatch[1]);
      const openMinute = parseInt(startMatch[2]);
      const closeHour = parseInt(endMatch[1]);
      const closeMinute = parseInt(endMatch[2]);

      // Determine schedule entry type based on event type
      // type 1 = Regular (theme park), type 2 = Water park, type 4 = Magical Halloween
      const isWaterPark = type === 2;
      const isSpecialEvent = type === 4; // Magical Halloween

      let scheduleEntryType, scheduleDescription;

      if (isWaterPark) {
        scheduleEntryType = 'INFO';
        scheduleDescription = 'Water Park';
      } else if (isSpecialEvent) {
        scheduleEntryType = 'INFO';
        scheduleDescription = event.description || 'Magical Halloween';
      } else {
        scheduleEntryType = 'OPERATING';
      }
      ...
      if (scheduleDescription) {
        scheduleEntry.description = scheduleDescription;
      }

      // Process each day range
      days.ranges.forEach(dayIndex => {
        // The dayIndex appears to be calculated as: monthIndex * 31 + dayOfMonth - 1
        // where monthIndex is 0-based (0=Jan, 1=Feb, etc.)
        const month = Math.floor(dayIndex / 31);
        const day = (dayIndex % 31) + 1;

        // Create date object
        const date = moment.tz([currentYear, month, day], this.config.timezone);

        // Skip invalid dates
        if (!date.isValid() || date.date() !== day) return;

        // Create opening and closing times
        const openingTime = date.clone().hour(openHour).minute(openMinute).second(0).millisecond(0);
        const closingTime = date.clone().hour(closeHour).minute(closeMinute).second(0).millisecond(0);
    ...
    // Sort schedule by date and opening time (to handle multiple venues per date)
    const finalSchedule = schedule.sort((a, b) => {
...
export class LegolandNewYork extends AttractionsIO {
  constructor(config = {}) {
    config.timezone = config.timezone || 'America/New_York';
    config.destinationId = config.destinationId || 'legolandnewyorkdestination';
    config.parkId = config.parkId || 'legolandnewyork';
    config.initialDataVersion = config.initialDataVersion || '2025-08-29T04:23:53Z';

    config.appBuild = config.appBuild || 217;
    config.appVersion = config.appVersion || '1.4.4';
    super(config);
  }
}

export class LegolandKorea extends AttractionsIO {
  constructor(config = {}) {
    config.timezone = config.timezone || 'Asia/Seoul';
    config.destinationId = config.destinationId || 'legolandkoreadestination';
    config.parkId = config.parkId || 'legolandkorea';
    config.initialDataVersion = config.initialDataVersion || '2025-08-20T05:38:01Z';

    config.appBuild = config.appBuild || 183;
    config.appVersion = config.appVersion || '1.2.3';
    super(config);
  }
}

export class PeppaPigThemeParkFlorida extends AttractionsIO {
  constructor(config = {}) {
    config.timezone = config.timezone || 'America/New_York';
    config.destinationId = config.destinationId || 'peppapigthemeparkfloridadestination';
    config.parkId = config.parkId || 'peppapigthemeparkflorida';
    config.initialDataVersion = config.initialDataVersion || '2025-09-23T16:20:01Z';

    config.appBuild = config.appBuild || 63;
    config.appVersion = config.appVersion || '1.0.16';
    super(config);
  }
}
