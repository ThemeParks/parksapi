...
export class DjursSommerland extends AttractionsIO {
  ...
  async buildEntityScheduleData() {
    ...
    // Sort schedule by date and remove duplicates (keep the last occurrence)
    const scheduleMap = new Map();
    schedule.forEach(entry => {
      scheduleMap.set(entry.date, entry);
    });
    
    const finalSchedule = Array.from(scheduleMap.values()).sort((a, b) => a.date.localeCompare(b.date));
