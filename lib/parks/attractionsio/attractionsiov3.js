...
export class AttractionsIOV3 extends Destination {
  ...
  async getParkEntranceLocation() {
    ...
    const searchForPOI = (name) => {
      const poiEnt = parkConfig.find((poi) => poi && poi.name == name);
    ...
    const entrance = searchForPOI("Main Entrance");
    if (entrance) {
      return entrance;
    }

    // also look for "Accessible Gate"
    const accessibleGate = searchForPOI("Accessible Gate");
    if (accessibleGate) {
      return accessibleGate;
    }
