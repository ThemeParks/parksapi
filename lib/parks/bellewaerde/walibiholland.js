...
export class WalibiHolland extends Destination {
  ...
  buildBaseEntityObject(data) {
    ...
    if (data) {
      // This is how they do IDs?!
      if (data.waitingTimeName) entity._id = data.waitingTimeName;
  ...
  async buildAttractionEntities() {
    // get POI data
    const poi = await this.fetchPOI();

    // build entities
    return poi.filter((x) => {
      // API seems to have no concept of IDs for things...
      // use waittime ID for our attraction ID, ignoring anything without a wait time for now
      return !!x.waitingTimeName;
    }).map((x) => {
