...
export class UniversalResortBase extends Destination {
  ...
  constructor(options = {}) {
    ...
    // api.universalparks.com client credentials (for getting API endpoint token)
    options.apiClientId = options.apiClientId || '';
    options.apiClientSecret = options.apiClientSecret || '';
    ...
    this.http.injectForDomain({
      hostname: "api.universalparks.com",
    }, async (method, url, data, options) => {
      if (options.loginRequest) {
      ...
      // add token as a Bearer token
      const token = await this.getApiEndpointToken();
      if (!token) {
        throw new Error('Failed to get service token for Universal API');
      }
  ...
  async getApiEndpointToken() {
    let tokenExpiration = null;
    return this.cache.wrap('universal_api_endpoint_token', async () => {
      // generate authorisation header for client credentials
      const authString = Buffer.from(`${this.config.apiClientId}:${this.config.apiClientSecret}`).toString('base64');

      const resp = await this.http('POST', 'https://api.universalparks.com/oidc/connect/token', 'scope=default&grant_type=client_credentials', {
        headers: {
      ...
      const expireTime = resp.body.expires_in * 1000;
  ...
  getRideIDFromWaitTimeId(poiData, waitTimeId) {
    try {
      // search all subkeys in poiData, not just Rides
      const ride = Object.values(poiData).flat().find((x) => {
        return x.ExternalIds && x.ExternalIds.PlaceId === waitTimeId || x.ExternalIds && x.ExternalIds.ContentId === waitTimeId;
      });
      if (!ride) {
        return null;
      }
  ...
  async buildEntityLiveData() {
    ...
    // gather POI data ready for later
    ...
    waittime.forEach((attraction) => {
      ...
      attraction.queues.forEach((queue) => {
        let rideIdObj = queue.alternate_ids.find((x) => {
        ...
        if (!rideIdObj) {
          if (attraction.wait_time_attraction_id) {
            rideIdObj = {
              system_name: 'POI',
              system_id: this.getRideIDFromWaitTimeId(poiData, attraction.wait_time_attraction_id),
            };
          } else {
            this.log(`No rideId found for attraction ${attraction.name} [${attraction.wait_time_attraction_id}] queue ${queue.queue_type}`);
            return;
          }
        }
        ...
        switch (queue.queue_type) {
          case 'STANDBY':
            ...
            // we don't really have a way to show at capacity
            //  show as "down", but with a wait time, if available
            if (queue.status == 'AT_CAPACITY') {
              attractionLiveDataObject.status = statusType.down;
              if (queue.display_wait_time) {
                attractionLiveDataObject.waitTime = queue.display_wait_time;
              } else {
                attractionLiveDataObject.waitTime = null;
              }
            }

            // DEBUG - gather various potential queue status types
            if (queue.status != 'OPEN' && queue.status != 'OPENS_AT' && queue.status != 'CLOSED' && queue.status != 'CLOSES_AT' && queue.status != 'BRIEF_DELAY' && queue.status != 'N/A' && queue.status != 'RIDE_NOW' && queue.status != 'WEATHER_DELAY' && queue.status != 'VIRTUAL_LINE_ONLY' && queue.status != 'EXTENDED_CLOSURE' && queue.status != 'AT_CAPACITY' && queue.status != 'COMING_SOON') {
          ...
          case 'SINGLE':
            ...
            if (attraction.has_single_rider) {
              if (queue.status == 'OPEN') {
                ...
                // single rider queues
                // display_wait_time is unreliable for single rider (often higher than standby)
                // and not shown in the official app, so report null
                attractionLiveDataObject.queue[queueType.singleRider] = {
                  waitTime: null,
                };
                attractionHasOperatingQueue = true;
              } else if (queue.status == 'AT_CAPACITY') {
                // TODO - we don't have a way to show single rider queues are at capacity
                //  so show nothing
              }
            }
            ...
            break;
          case 'EXPRESS':
            // Universal Express (paid skip-the-line)
            // status field is not reliable (always CLOSED), and display_wait_time values
            // are inconsistent, so report availability only (null wait time)
            // 995 = sentinel for unavailable, any other number = express is available
            if (queue.display_wait_time !== undefined && queue.display_wait_time !== 995) {
              if (!attractionLiveDataObject.queue) {
                attractionLiveDataObject.queue = {};
              }

              attractionLiveDataObject.queue[queueType.paidStandBy] = {
                waitTime: null,
              };
            }
    ...
    // add in any missing wait times available in the POI data
    const catsToSearch = ['Rides', 'Shows'];
    catsToSearch.forEach((cat) => {
      poiData[cat].forEach((ride) => {
        const rideId = ride.Id.toString();
        const rideLiveDataObject = returnLiveData.find((x) => x._id === rideId);
        if (rideLiveDataObject) {
          if (rideLiveDataObject.queue && rideLiveDataObject.queue[queueType.standBy]) {
            // we already have live data for this ride, skip
            return;
          }
        }

        // check out the ride.WaitTime something
        if (ride.WaitTime !== undefined && ride.WaitTime !== null) {
          if (ride.WaitTime >= 0) {
            const rideEntry = findOrCreateLiveData(ride.Id.toString());
            rideEntry.status = statusType.operating;
            rideEntry.queue = {};
            rideEntry.queue[queueType.standBy] = {
              waitTime: ride.WaitTime,
            };
          } else {
            // negative wait time means closed
            // TODO - any special states?
            //  -2 = Delayed
            //  -4 = Weather Delay
            //  -6 = closed (out of hours)
            const rideEntry = findOrCreateLiveData(ride.Id.toString());
            if (ride.WaitTime === -4 || ride.WaitTime === -2) {
              rideEntry.status = statusType.down;
            } else if (ride.WaitTime === -6) {
              rideEntry.status = statusType.closed;
            } else {
              // unknown state
              // rideEntry.status = statusType.closed;
            }
