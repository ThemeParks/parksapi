...
export class Park extends ConfigBase {
  ...
  constructor(options = {}) {
    ...
    // use this for any "offline mode" Park objects using pure-cache data
    this._hasOfflineData = false;
    this._offlineCalendar = null;
  ...
  /**
   * Is this Park object operating offline?
   */
  get offline() {
  ...
  /**
   * Archive any data that is needed for serialisation of this Park.
   * @param {object} ar The archive object to write/load data to/from
   * @param {boolean} saving Is this archive saving or loading?
   * @private
   */
  _archive(ar, saving = true) {}

  /**
   * Serialise this Park object
  ...
  async serialise(bundle = {}, saving = true, options = {
    ...
    if (saving) {
      ...
      // save basic Park data
      bundle.ar.attractions = await this.getAttractions();
      bundle.ar.calendar = await this.getCalendar();
    } else {
      // === Loading ===
      ...
      // load basic Park data
      this._attractions = bundle.ar.attractions;
      this._offlineCalendar = bundle.ar.calendar;
    }

    // call any custom archive operations for this park
    this._archive(bundle, saving);
    ...
    // after loading, check if our date has changed based on any new calendar data
    this._checkDate();
   ...
   */
  async updateAttractionQueue(attractionID, waitTime = undefined, queueType = queueType.standBy) {
    if (attractionID === undefined) return;
    ...
    if (existingRide) {
      ...
      if (waitTime === undefined) {
        if (existingRide.queue[queueType] !== undefined) {
          const previousWaitTime = existingRide.queue[queueType].waitTime;
          delete existingRide.queue[queueType];
   ...
   */
  async getCalendar() {
    if (this._offlineCalendar !== null) {
      return this._offlineCalendar;
    }
