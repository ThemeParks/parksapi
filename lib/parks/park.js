...
export class Park extends ConfigBase {
  ...
  async getCalendar() {
    ...
    // populate from yesterday (if the API even gives us yesterday)
  ...
  async getCalendarForToday() {
    ...
    const isInParkHours = [
      ...
      if (!parkHours) return false;
      const isInAnyParkHours = parkHours.find((hours) => {
    ...
    if (isInParkHours.length === 0) {
      ...
      return calendar[nowInPark.format('YYYY-MM-DD')];
    }
    // otherwise return the hours that we currently match
    return isInParkHours[0];
  }
