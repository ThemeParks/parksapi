    ...
    if (existingRide) {
      ...
      if (!existingRide.queue[type]) {
        ...
        if (type === queueType.standBy) {
          ...
          existingRide.queue[type].state = returnTimeState.finished;
        }
      }
      ...
      if (type == queueType.standBy || type == queueType.singleRider) {
        ...
        if (
          !queueValue.returnStart ||
          !queueValue.returnEnd ||
          Object.values(returnTimeState).indexOf(queueValue.state) < 0
        ) {
          this.emit(
              'error',
        ...
        queueValue.returnStart = moment.tz(queueValue.returnStart, this.config.timezone).format();
        queueValue.returnEnd = moment.tz(queueValue.returnEnd, this.config.timezone).format();
