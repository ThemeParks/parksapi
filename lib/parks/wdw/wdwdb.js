...
export default class DisneyLiveDB extends ConfigBase {
   ...
   */
  async getDocsByIndex(indexes) {
    await this.init();

    return (await Promise.all(indexes.map((id) => {
  ...
  async findIndexEntity(id, filter = {}) {
    ...
    // TODO - filter entries by supplied filter options before resolving
  ...
  async findEntity(id, filter = {}) {
    ...
    return this.getDocsByIndex(entities.map((entry) => {
      return entry._id;
    }));
  }
   ...
   */
  async findOne(id) {
    const entities = await this.findIndexEntity(id);
  ...
  async findByChannel(channel) {
    ...
    return this.getDocsByIndex(channelIndex.map((entry) => {
      return entry._id;
    }));
  }
}
