 ...
 */
  ...
  _replicate() {
    ...
    const noChangeTimer = 5; // how many minutes before killing and restarting replication
    ...
    try {
      replicationHandle = PouchDB.replicate(this.remoteDB, this, {
        ...
        //  if the timer is ever first, we will restart the replicator
        if (noChangeTimeout) {
          clearTimeout(noChangeTimeout);
        }
  ...
  _extractEntityKeys(doc) {
    ...
    if (doc.channels[0].indexOf('.calendar.') >= 0) {
