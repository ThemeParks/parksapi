...
export class DisneyLiveResort extends Resort {
  ...
  async getRestaurantMenu(id) {
    ...
    return this.cache.wrap(
      ...
      async () => {
        try {
          const menu = await this.http('GET', `https://dining-menu-svc.wdprapps.disney.com/diningMenuSvc/orchestration/menus/${id}`);
          if (!menu) return undefined;

          const menuData = menu.body.menus.map((menuGroup) => {
            const items = [];
            ...
            menuGroup.menuGroups.forEach((group) => {
              group.menuItems.forEach((dish) => {
                items.push({
                  ...
                  price: {
                    // TODO - figure out pricing structure
                    USD: dish?.prices?.PerServing?.withoutTax * 100 || null,
                  },
                });
              });
            });
      ...
      1000 * 60 * 60 * 3, // cache for 3 hours
    );
  }
