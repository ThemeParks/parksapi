...
export class DisneyLiveResort extends Resort {
  ...
  async _buildLiveDataObject(doc) {
    ...
    // facility status is a virtual queue!
    if (doc.status == 'Virtual Queue') {
      // get vqueue data for this resort
      ...
      if (vQueueData) {
        ...
        if (attractionVQueueData) {
          ...
          // TODO - more specific status for temporarily closed?
          if (!data.queue) data.queue = {};
          data.queue[queueType.boardingGroup] = {
            status: attractionVQueueData.state === 'PAUSED' ?
              boardingGroupState.closed : boardingGroupState.available,
