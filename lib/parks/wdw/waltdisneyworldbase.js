...
export class DisneyLiveResort extends Resort {
  ...
  async _buildLiveDataObject(doc) {
    ...
    */
    ...
    /*
    // TODO - access wdw.today.1_0.Attraction for attraction refurbs
    // check our refurb data and override any incoming status
    if (this.refurbs.find((x) => x.entityID === entityID) !== undefined) {
      status = statusType.refurbishment;
    }
  ...
  async buildEntityLiveData() {
    ...
    // TODO - fetch show times
    const entertainmentToday = await this.db.get(`${this.resortShortcode}.today.1_0.Entertainment`);
    // TODO - if we have no facility times, can we remove livedata from old entities?
    if (entertainmentToday && entertainmentToday.facilities) {
      Object.keys(entertainmentToday.facilities).forEach((facId) => {
        // look for existing live data doc that has a facility status entry
        let liveData = docs.find((x) => x._id === facId);
        if (!liveData) {
          // otherwise, create a doc
          liveData = docs[docs.push({
            _id: facId,
          }) - 1];
        }

        // inject live showtimes into entity
        liveData.showtimes = entertainmentToday.facilities[facId].filter((x) => {
