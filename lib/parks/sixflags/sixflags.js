...
export class SixFlags extends Destination {
  ...
  async buildDestinationEntity() {
    ...
    return destinations.map(destination => {
      return {
        ...
        _id: `destination_${destination.parkId}`,
        slug: destination.name.replace(/\s+/g, '-').toLowerCase(),
  ...
  async buildParkEntities() {
    ...
    return destinations.map((destination) => {
      return {
        ...
        _destinationId: `destination_${destination.parkId}`,
        _parentId: `destination_${destination.parkId}`,
    ...
    for (const park of parksToFetch) {
      ...
      attractionEntities.push(...ridePOI.rides.map(attraction => {
        return {
          ...
          _destinationId: `destination_${park}`,
  ...
  async buildShowEntities() {
    ...
    for (const park of parksToFetch) {
      ...
      showEntities.push(...showsPOI.shows.map(show => {
        return {
