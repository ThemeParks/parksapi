  ...
  constructor(options = {}) {
    ...
    this.http.injectForDomain({
      ...
      if (!options.skipAuth) {
        ...
        // increase timeout for all requests to Six Flags API
        options.response_timeout = 60 * 1000; // 60 seconds timeout for all Six Flags API requests
      }
    });
    ...
    for (const park of parksToFetch) {
      ...
      liveData.push(...parkData.rideStatuses.map(ride => {
        ...
        switch (ride.status) {
          ...
          case 'AttractionStatusTemporarilyClosedDueToWeather':
