...
export class SixFlags extends Destination {
  ...
  buildBaseEntityObject(data) {
    ...
    if (!entity.timezone && entity.location && entity.location.latitude && entity.location.longitude) {
      try {
  ...
  async buildDestinationEntity() {
    ...
    for (const park of parks) {
      ...
      const entity = {
        ...
        _id: `sixflags__destination_${park.parkId}`,
  ...
  async buildParkEntities() {
    ...
    for (const park of parks) {
      ...
      const entity = {
        ...this.buildBaseEntityObject(location ? {location} : null),
        _id: `sixflags__park_${park.parkId}`,
        _destinationId: `sixflags__destination_${park.parkId}`,
        _parentId: `sixflags__destination_${park.parkId}`,
        name: park.name,
  ...
  async buildAttractionEntities() {
    ...
    for (const parkID of parkIDs) {
      ...
      const destinationEntity = destinations.find(d => d._id === `sixflags__destination_${parkID}`);

      // Filter for rides (venueId: 1) and ensure they belong to this park
      const rides = poiData.filter(poi => poi.venueId === 1 && poi.parkId === parkID);

      attractionEntities.push(...rides.map(attraction => {
        const entity = {
          ...this.buildBaseEntityObject(attraction),
          _id: attraction.fimsId,
          _parkId: `sixflags__park_${parkID}`,
          _parentId: `sixflags__park_${parkID}`,
          _destinationId: `sixflags__destination_${parkID}`,
  ...
  async buildShowEntities() {
    ...
    for (const parkID of parkIDs) {
      ...
      const shows = poiData.filter(poi => poi.venueId === 2 && poi.parkId === parkID);
  ...
  async buildRestaurantEntities() {
    ...
    for (const parkID of parkIDs) {
      ...
      const restaurants = poiData.filter(poi => poi.venueId === 4 && poi.parkId === parkID);
  ...
  async buildEntityLiveData() {
    ...
    for (const parkID of parkIDs) {
      ...
      ridesVenueStatus.details.forEach(ride => {
        ...
        // Get wait time from wait times data first (more accurate than venue status)
        ...
        // Determine status: if we have a valid wait time, ride is operating
        if (waitTime != null && waitTime >= 0) {
          rideLiveData.status = statusType.operating;
        } else {
          // Fall back to venue status if no wait time data
          const venueStatusStr = ride.status?.toLowerCase() || '';

          if (venueStatusStr.includes('open') || venueStatusStr === 'on time') {
            rideLiveData.status = statusType.operating;
          } else if (venueStatusStr.includes('closed') || venueStatusStr === 'not scheduled') {
            rideLiveData.status = statusType.closed;
          } else if (venueStatusStr.includes('down') || venueStatusStr.includes('temporarily')) {
            rideLiveData.status = statusType.down;
          } else if (venueStatusStr === '') {
            // No status from venue, default to closed
            rideLiveData.status = statusType.closed;
          }
          // If none of the conditions match, status remains at default (closed)
        }
        ...
        // Add fast lane queue if available (only if wait time > 0)
        if (waitTimeInfo?.isFastLane && waitTimeInfo.fastlaneWaittime?.waitTime != null) {
      ...
      if (showsVenueStatus && showsVenueStatus.details) {
        ...
        const parkEntity = await this.getParkEntities().then(entities =>
          entities.find(x => x._id === `park_${parkID}`)
        );
  ...
  async buildEntityScheduleData() {
    ...
    for (const parkID of parkIDs) {
      const parkEntity = parkEntities.find(x => x._id === `park_${parkID}`);
