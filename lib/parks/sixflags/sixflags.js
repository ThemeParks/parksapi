...
export class SixFlags extends Destination {
  ...
  async _getParkData() {
    ...
    // Extract theme parks (showThemePark === true)
    const themeParkEntries = Object.entries(parkSettings)
      .filter(([, settings]) => {
        return settings.showThemePark === true;
      });

    const parkIds = themeParkEntries.map(([parkId, settings]) => ({
      parkId: parseInt(parkId),
      code: settings.code,
    }));

    // Extract water parks from otherParks config
    for (const [parentParkId, settings] of themeParkEntries) {
      if (settings.otherParks) {
        for (const otherPark of settings.otherParks) {
          if (otherPark.label === 'Water Park' && otherPark.fimsId && otherPark.fimsSiteCode) {
            parkIds.push({
              parkId: parseInt(otherPark.fimsId),
              code: otherPark.fimsSiteCode,
              parentCode: settings.code,
              parentParkId: parseInt(parentParkId),
              waterParkName: otherPark.subProperty,
            });
          }
        }
      }
    ...
    const parks = await Promise.all(parkIds.map(async (park) => {
      let name = park.waterParkName || parkNamesMap[park.parkId];
      ...
      if (park.parentCode) {
        result.parentCode = park.parentCode;
        result.parentParkId = park.parentParkId;
      }
  ...
  async buildDestinationEntity() {
    ...
    for (const park of parks) {
      // Water parks share their parent's destination, skip them
      if (park.parentCode) continue;
  ...
  async buildParkEntities() {
    ...
    for (const park of parks) {
      ...
      // Water parks use their parent's destination
      const destinationCode = park.parentCode || park.code;

      const entity = {
        ...this.buildBaseEntityObject(location ? {location} : null),
        _id: `sixflags_park_${park.code}`,
        _destinationId: `sixflags_destination_${destinationCode}`,
        _parentId: `sixflags_destination_${destinationCode}`,
        name: park.name,
  ...
  async buildAttractionEntities() {
    ...
    for (const park of parks) {
      // Water park POIs are served through the parent park's endpoint
      const poiParkId = park.parentParkId || park.parkId;
      const poiData = await this.fetchPOI({parkID: poiParkId});

      if (!poiData || !Array.isArray(poiData)) {
        continue;
      }

      // Water parks use their parent's destination for fallback location
      const destinationCode = park.parentCode || park.code;
      const destinationEntity = destinations.find(d => d._id === `sixflags_destination_${destinationCode}`);
  ...
  async buildEntityScheduleData() {
    ...
    for (const park of parks) {
      ...
      const parkSchedule = allDates.map(dateObj => {
        ...
        // Find rides venue to determine park hours, fall back to any venue with hours
        const ridesVenue = dateObj.venues?.find(v => v.venueId === 1 && v.detailHours?.length > 0)
          || dateObj.venues?.find(v => v.detailHours?.length > 0);
