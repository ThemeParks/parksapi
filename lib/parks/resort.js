...
const metaCommands = {
  ...
  cache: function (fnName, args) {
    const originalFunction = this[fnName];
...
export class Resort extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    if (!this.config.timezone) {
      throw new Error(`All resort objects must have a timezone! ${this.constructor.name}`);
    }
  ...
  /**
   * Return the current time for this resort in its local timezone
   * @return {moment}
   */
  getTimeNowMoment() {
