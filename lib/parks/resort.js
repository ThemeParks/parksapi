...
export class Resort extends EventEmitter {
   ...
   * @private
   */
  async setLiveCache(id, data) {
    await this.cache.set(`${id}_live`, data);
    // generate hash and store separately
    await this.cache.set(`${id}_livehash`, objHash(data));
  }
   ...
   * @private
   */
  async getLiveHash(id) {
    return await this.cache.get(`${id}_livehash`);
  }
  ...
  async updateEntityLiveData(internalId, data) {
    ...
    const storedHash = await this.getLiveHash(internalId);
    ...
    // store locally
    await this.setLiveCache(internalId, data);
  }
