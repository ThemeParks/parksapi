...
export class Resort extends EventEmitter {
  ...
  async setLiveCache(id, data, lock) {
    ...
    await cache.set(`${id}_live`, data);
    // generate hash and store separately
    await cache.set(`${id}_livehash`, objHash(data));
  }
  ...
  async updateEntityLiveData(internalId, data) {
    ...
    const lock = this.cache.createLock();
    ...
    try {
      ...
      if (storedHash) {
        ...
        if (newHash === storedHash) {
          ...
          await lock.rollback(); // release lock before returning!
          return;
        }
      }
      ...
      console.error(e);
      // rollback our DB lock if anything fails
      await lock.rollback();
    }
  }
