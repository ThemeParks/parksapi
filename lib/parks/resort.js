...
export class Resort extends EventEmitter {
  ...
  async updateEntityLiveData(internalId, data) {
    ...
    if (!entity) {
      throw new EntityNotFound(`Trying to assign live data update to unknown entity ${internalId}`);
    }
    ...
    if (validationErrors !== null) {
      ...
      throw new Error(
          `Error validating incoming live data [${internalId}] ${JSON.stringify(data)}.\n\t${validationErrors.map((x) => `${x.dataPath} ${x.message}`).join('\n\t')}`,
      );
    }
