...
export class ShanghaiDisneylandResort extends Destination {
  ...
  async _fetchWaitTimes() {
    ...
    const waitTimes = await this.http(
      'GET',
      `${this.config.apiBase}resource-assembler-platform/public/wait-times/shdr;entityType=destination?region=cn`,
      undefined,
      {json: true},
    );
  ...
  async _fetchUpcomingCalendar() {
    ...
    return (await this.http(
      'GET',
      `${this.config.apiBase}resource-assembler-platform/public/ancestor-activities-schedules/shdr;entityType=destination`,
      {
        filters: 'resort,theme-park,water-park,land,Entertainment-Venue,Attraction,Entertainment,Event,Spa,tour,recreation-activity,Recreation,restaurant,MerchandiseFacility,guest-service',
  ...
  async buildEntityScheduleData() {
    ...
    const parkActivities = cal.activities.filter((entity) => {
      const entData = this.extractEntityData(entity);
      return entData.entityType === 'theme-park' || entData.entityType === 'water-park';
    });

    return parkActivities.map((entity) => {
