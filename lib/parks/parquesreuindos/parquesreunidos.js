      ...
      try {
        ...
        for (const label of parsedLabels) {
          ...
          for (const key in label) {
            // try to parse hours from label. Will be of format similar to "10am - 5pm"
            //  if we can't parse, skip this label (special event, or closure)
            const regex = /(\d{1,2})(am|pm)\s*-\s*(\d{1,2})(am|pm)/;
            const match = label[key].match(regex);
            if (match) {
              const startHour = moment(match[1], 'h').add(match[2] === 'pm' ? 12 : 0, 'hours');
              const endHour = moment(match[3], 'h').add(match[4] === 'pm' ? 12 : 0, 'hours');

              // hack - 12pm == 12:00, not midnight
            ...
            // other label types
            //  dutch (eg. Bobbejaanland)
            // eg.
            //  10 tot 19 u
            //  13 tot 18u
            const regex2 = /(\d{1,2})\s*tot\s*(\d{1,2})\s*u/;
            const match2 = label[key].match(regex2);
            if (match2) {
              const startHour = moment(match2[1], 'h').add(0, 'hours');
              const endHour = moment(match2[2], 'h').add(0, 'hours');
        ...
        this.log.error('Error parsing schedule labels data', e);
      }
    }
