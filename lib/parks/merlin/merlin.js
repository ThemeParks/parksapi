...
export class AttractionsIO extends Destination {
  ...
  async buildEntityScheduleData() {
    ...
    const hoursRegex = /(\d{1,2})([a|p]m)\s*\-\s*(\d{1,2})([a|p]m)/;

    const schedule = days.map((x) => {
      const date = moment(x.key, 'YYYYMMDD').tz(this.config.timezone, true);

      const times = hoursRegex.exec(x.openingHours);
      if (!times) return;

      const openingTime = date.clone().hour(Number(times[1]) + (times[2] === 'pm' ? 12 : 0)).minute(0).second(0).millisecond(0);
      const closingTime = date.clone().hour(Number(times[3]) + (times[4] === 'pm' ? 12 : 0)).minute(0).second(0).millisecond(0);;
