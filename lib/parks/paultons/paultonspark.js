...
export class PaultonsPark extends Destination {
  ...
  /**
   * Convert an ORMS ID to an entity ID
   * @param {number} ormsId 
   * @returns {string|null}
   */
  async _ormsIdToEntityId(ormsId) {
    const poi = await this.fetchPOIData();
    const ent = poi.data.find((x) => x.orms_id == ormsId);
    if (!ent) {
      return null;
    }
    return `${ent.id}`;
  ...
  /**
   * Helper function to build a list of entities from a filter
   * @param {*} types 
   * @param {*} extraData 
  ...
  async buildEntityLiveData() {
    ...
    for (const ride of liveData) {
      // find the ride in our POI data
      const rideId = await this._ormsIdToEntityId(ride?.rideId);

      // if no ride ID, skip, this attraction isn't published
      //  usually a defunct ride, seasonal attraction, or (bizarely) the car park
      if (!rideId) {
        continue;
      }
      ...
      liveObj.status = !!ride.statusOpen ? statusType.operating : statusType.closed;

      // TODO - opening times?

      // queue time, if available
      if (ride.queueTime != null && ride.queueTime != undefined) {
        liveObj.queue = {
          [queueType.standBy]: {
            waitTime: ride.queueTime,
          }
        };
      }

      // optionally add opening hours
      if (ride.updatedAt && ride.closingTime && ride.openingTime) {
        ...
        const date = ride.updatedAt.substring(0, 10);

        const openingTimeStr = `${date}T${ride.openingTime}`;
        const closingTimeStr = `${date}T${ride.closingTime}`;
