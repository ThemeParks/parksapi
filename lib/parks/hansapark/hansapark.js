...
export class HansaPark extends Destination {
  ...
  async buildEntityLiveData() {
    ...
    const attractionLiveData = poi.data.filter((x) => {
    ...
    const anyValidData = !!attractionLiveData.find((x) => {
      if (!x.isOpen) return false;
      if (x.outOfOrder || x.isOutOfOrder) return false;
      if (x?.waitingTime?.minutes) {
        return true;
      }
      return false;
    });

    const liveData = attractionLiveData.map((x) => {
      ...
      if (liveObj.status == statusType.operating) {
        liveObj.queue = {
          [queueType.standBy]: {
            waitTime: anyValidData && x.waitingTime.minutes !== undefined ? x.waitingTime.minutes : null,
          },
        };
      }
