...
export class Phantasialand extends Destination {
  ...
  async getCalendarJSON() {
    ...
    // loop over next 90 days
    const calendarData = [];
    for (let i = 0; i < 90; i++) {
      const dateObj = today.clone().add(i, 'days');
      const date = dateObj.format('YYYY-MM-DD');

      // if date is before or equal to november 14th 2025, we can use 9 - 18
      //  otherwise, ignore, we will sort it out later
      if (dateObj.isBefore('2025-11-14')) {
        calendarData.push({
          date: date,
          title: '9:00 18:00',
        });
