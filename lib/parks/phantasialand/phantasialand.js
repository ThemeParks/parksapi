import {Destination} from '../destination.js';
import {attractionType, statusType, queueType, tagType, scheduleType, entityType} from '../parkTypes.js';

import {v4 as uuidv4} from 'uuid';

import sift from 'sift';
import moment from 'moment-timezone';

//import cheerio from 'cheerio';

export class Phantasialand extends Destination {
  constructor(options = {}) {
    options.timezone = options.timezone || 'Europe/Berlin';

    options.apiBase = options.apiBase || '';
    
    super(options);

    if (!this.config.apiBase) throw new Error('Missing API Base URL (apiBase)');

    const baseURLHostname = new URL(this.config.apiBase).hostname;

    this.http.useragent = 'okhttp/3.12.1';

    this.http.injectForDomain({
      hostname: baseURLHostname,
    }, async (method, url, data, options) => {
      const isLoginRequest = url.indexOf('app-users') >= 0;
      if (!isLoginRequest && method === 'GET') {
        // fetch our cached access token
        data.access_token = await this.fetchAccessToken();
      }
    });

    // similarly, we can also inject into HTTP responses
    //  if we detect an unauthorised response, we can unset our local auth tokens so they are refetched
    this.http.injectForDomainResponse({
      hostname: baseURLHostname,
    }, async (response) => {
      // look for 401 HTTP code (unauthorised)
      if (response.statusCode === 401) {
        // clear our access token on unauthorised responses
        await this._clearFunctionCache('fetchAccessToken');
        return undefined;
      }

      // otherwise, return the actual response
      return response;
    });
  }

  /**
   * Create or fetch existing user for app login
   */
  async createUser() {
    '@cache|481801'; // cache 11 months

    // generate login
    const username = `${uuidv4()}@android.com`;
    const password = uuidv4();

    // create our user
    const userCreationResp = await this.http(
      'POST',
      `${this.config.apiBase}app-users`,
      {
        "email": username,
        "language": "en",
        "password": password,
        "platform": "android"
      },
      {
        json: true,
      },
    );

    if (userCreationResp.body.email !== username) {
      return undefined;
    }

    return {
      "email": username,
      "password": password,
    };
  }

  /**
   * Fetch cached access token, or fetch new one if expired
   * @returns {string}
   */
  async fetchAccessToken() {
    '@cache|481801'; // cache 11 months

    // create user
    const user = await this.createUser();
    if (user === undefined) {
      return undefined;
    }

    // login and get access token
    const accessTokenResp = await this.http(
      'POST',
      `${this.config.apiBase}app-users/login`,
      {
        "email": user.email,
        "password": user.password,
        "ttl": 31556926, // 1 year, matching app behaviour
      },
      {
        json: true,
      },
    );

    this.log('Got new access token', accessTokenResp?.body?.id);

    // return our new access token
    return accessTokenResp?.body?.id;
  }

  /**
   * Get the POI data objects from the API
   * @returns {Array<Object>}
   */
  async getPOIData() {
    '@cache|360'; // cache 6 hours
    const POI = await this.http(
      'GET',
      `${this.config.apiBase}pois`,
      {
        'filter[where][seasons][like]': '%',
        compact: true,
      },
    );

    return POI.body;
  }

  /**
   * Helper function to build a basic entity document
   * Useful to avoid copy/pasting
   * @param {object} data 
   * @returns {object}
   */
  buildBaseEntityObject(data) {
    const entity = super.buildBaseEntityObject(data);

    if (data) {
      if (data.adminOnly) return undefined;

      // not on the map, ignore
      //if (!data.poiNumber) {
      //  return undefined;
      //}

      // if has no seasons, ignore
      if (!data.seasons || data.seasons.length === 0) {
        return undefined;
      }

      if (data.id !== undefined) {
        entity._id = `${data.id}`;
      }

      // entity name
      entity.name = data.title?.en || data._title?.en || data.title?.de || data._title?.de || undefined;

      // entity location
      const location = data.entrance?.world || data._entrance?.world;
      if (location) {
        entity.location = {
          longitude: location.lng,
          latitude: location.lat,
        };
      }

      // tags
      entity._tags = [];

      if (Array.isArray(data.tags)) {
        if (data.tags.includes('ATTRACTION_TYPE_WATER')) {
          entity._tags.push({id: tagType.mayGetWet, value: true});
        }
        if (data.tags.includes('ATTRACTION_TYPE_SINGLE_RIDER_LINE')) {
          entity._tags.push({id: tagType.singleRider, value: true});
        }
        if (data.tags.includes('ATTRACTION_TYPE_PICTURES')) {
          entity._tags.push({id: tagType.onRidePhoto, value: true});
        }
        if (data.tags.includes('ATTRACTION_TYPE_QUICK_PASS') || data.tags.includes('ATTRACTION_TYPE_QUICK_PASS_PLUS')) {
          entity._tags.push({id: tagType.fastPass, value: true});
        }
      }

      if (data.minSize) {
        entity._tags.push({id: tagType.minimumHeight, value: {height: data.minSize, unit: 'cm'}});
      }
      if (data.maxSize) {
        entity._tags.push({id: tagType.maximumHeight, value: {height: data.maxSize, unit: 'cm'}});
      }
    }

    return entity;
  }

  /**
   * Build the destination entity representing this destination
   */
  async buildDestinationEntity() {
    return {
      ...this.buildBaseEntityObject(null),
      _id: 'phantasialanddest',
      slug: 'phantasialand', // all destinations must have a unique slug
      name: 'Phantasialand',
      entityType: entityType.destination,
      location: {
        longitude: 6.879291227409914,
        latitude: 50.798995255201866,
      },
    };
  }

  /**
   * Build the park entities for this destination
   */
  async buildParkEntities() {
    return [
      {
        ...this.buildBaseEntityObject(null),
        _id: 'phantasialand',
        _destinationId: 'phantasialanddest',
        _parentId: 'phantasialanddest',
        slug: 'phantasialandpark',
        name: 'Phantasialand',
        entityType: entityType.park,
        location: {
          longitude: 6.879291227409914,
          latitude: 50.798995255201866,
        }
      }
    ];
  }

  async _buildEntitiesFromCategory(filter, extraFields = {}) {
    const entities = await this.getPOIData();

    return entities.filter(sift(filter)).map((x) => {
      const ent = this.buildBaseEntityObject(x);
      if (!ent) return undefined;
      return {
        ...ent,
        _destinationId: 'phantasialanddest',
        _parentId: 'phantasialand',
        _parkId: 'phantasialand',
        ...extraFields,
      };
    }).filter((x) => !!x);
  }

  /**
   * Build the attraction entities for this destination
   */
  async buildAttractionEntities() {
    return await this._buildEntitiesFromCategory({
      category: 'ATTRACTIONS',
    }, {
      entityType: entityType.attraction,
      attractionType: attractionType.ride,
    });
  }

  /**
   * Build the show entities for this destination
   */
  async buildShowEntities() {
    return await this._buildEntitiesFromCategory({
      $or: [
        {category: 'SHOWS'},
        {category: 'THE_SIX_DRAGONS'},
        {category: 'THEATER'},
      ],
    }, {
      entityType: entityType.show,
    });
  }

  /**
   * Build the restaurant entities for this destination
   */
  async buildRestaurantEntities() {
    return await this._buildEntitiesFromCategory({
      $or: [
        {category: 'RESTAURANTS_AND_SNACKS'},
        {category: 'PHANTASIALAND_HOTELS_RESTAURANTS'},
      ],
      tags: {
        $elemMatch: {
          $eq: 'RESTAURANT',
        },
      },
    }, {
      entityType: entityType.restaurant,
    });
  }

  /**
   * Get a random lat/lon position within this destination
   */
  getRandomLocation() {
    return {
      longitude: 6.878342628 + (Math.random() * (6.877570152 - 6.878342628)),
      latitude: 50.800659529 + (Math.random() * (50.799683077 - 50.800659529)),
    };
  }

  /**
   * Fetch live data from API
   */
  async fetchLiveData() {
    '@cache|1'; // cache 1 minute
    const randomLoc = this.getRandomLocation();

    const waitData = await this.http('GET',
      `${this.config.apiBase}signage-snapshots`,
      {
        loc: `${randomLoc.latitude.toFixed(14)},${randomLoc.longitude.toFixed(14)}`,
      },
    );

    return waitData?.body;
  }

  /**
   * @inheritdoc
   */
  async buildEntityLiveData() {
    const liveData = await this.fetchLiveData();

    return liveData.map((x) => {
      const liveData = {
        _id: `${x.poiId}`,
        status: statusType.operating,
      };

      if (x.showTimes !== null) {
        liveData.status = x.showTimes.length > 0 ? statusType.operating : statusType.closed;
        liveData.showtimes = x.showTimes.map((x) => {
          return {
            type: "Performance Time",
            startTime: moment.tz(x, "YYYY-MM-DD HH:mm:ss", this.config.timezone).format(),
            // return null for endTime as we don't have a show length available
            endTime: null,
          };
        });
      }

      if (x.waitTime !== null) {
        liveData.status = x.open ? statusType.operating : statusType.closed;
        liveData.queue = {
          [queueType.standBy]: {
            waitTime: x.waitTime,
          },
        };
      }

      if (x.waitTime === null && x.showTimes === null && x.open !== null) {
        liveData.status = x.open ? statusType.operating : statusType.closed;
      }

      return liveData;
    }).filter((x) => !!x);
  }

  /**
   * Fetch live park info (isOpen, closing time) from API
   */
  async fetchParkInfos() {
    '@cache|5'; // cache 5 minutes
    const resp = await this.http('GET', `${this.config.apiBase}park-infos`, {});
    return resp?.body?.[0];
  }

  /**
   * Fetch calendar HTML data
   */
  async fetchCalendarData() {
    '@cache|120';
    const html = await this.http('GET', 'https://www.phantasialand.de/en/theme-park/opening-hours/');
    return html.body;
  }

  /**
   * Parse calendar data JSON from webpage
   */
  async getCalendarJSON() {
    '@cache|360'; // cache 6 hours

    const html = await this.fetchCalendarData();

    // Extract the data-calendar attribute from the HTML
    const calendarMatch = html.match(/data-calendar='(\[.*?\])'/s);
    if (!calendarMatch) {
      this.log('No calendar data found in HTML');
      return [];
    }

    // Parse the JSON data
    let calendarEvents;
    try {
      const jsonStr = calendarMatch[1]
        .replace(/&nbsp;/g, ' ')
        .replace(/&#125;/g, '}');
      calendarEvents = JSON.parse(jsonStr);
    } catch (e) {
      this.log('Failed to parse calendar JSON', e);
      return [];
    }

    // Convert the events format to our internal format
    const calendarData = [];

    for (const event of calendarEvents) {
      // Skip events with no selected days or empty titles
      if (!event.days_selected || event.days_selected.length === 0) {
        continue;
      }

      // Use English title to extract hours
      const title = event.title_en || event.title;

      // Handle closed days
      if (title.toLowerCase().includes('closed')) {
        for (const date of event.days_selected) {
          calendarData.push({
            date: date,
            title: 'Closed',
          });
        }
        continue;
      }

      // Extract hours from title (format like "09 a.m. until 06 p.m." or "11:00 – 20:00")
      // Try different patterns
      let hours = null;

      // Pattern 1: "09 a.m. until 06 p.m."
      const ampmMatch = title.match(/(\d{1,2})\s*a\.m\.\s*(?:until|–|to|-)\s*(\d{1,2})\s*p\.m\./i);
      if (ampmMatch) {
        const openHour = ampmMatch[1].padStart(2, '0');
        let closeHour = parseInt(ampmMatch[2]);
        if (closeHour !== 12) closeHour += 12;
        hours = `${openHour}:00 ${closeHour}:00`;
      }

      // Pattern 2: "11:00 – 20:00" or similar
      if (!hours) {
        const twentyFourMatch = title.match(/(\d{1,2}):(\d{2})\s*(?:–|–|-|to|until)\s*(\d{1,2}):(\d{2})/);
        if (twentyFourMatch) {
          hours = `${twentyFourMatch[1]}:${twentyFourMatch[2]} ${twentyFourMatch[3]}:${twentyFourMatch[4]}`;
        }
      }

      if (hours) {
        for (const date of event.days_selected) {
          calendarData.push({
            date: date,
            title: hours,
          });
        }
      }
    }

    return calendarData;
  }

  /**
   * Return schedule data for all scheduled entities in this destination
   * Eg. parks
   * @returns {array<object>}
   */
  async buildEntityScheduleData() {
    const [calendarData, parkInfos] = await Promise.all([
      this.getCalendarJSON(),
      this.fetchParkInfos(),
    ]);

    if (!calendarData) return [];

    const today = moment.tz(this.config.timezone).format('YYYY-MM-DD');
    const parkSchedule = [];
    const hoursRegex = /([0-9]{1,2}):([0-9]{2}).*\s([0-9]{1,2}):([0-9]{2})/;

    calendarData.forEach((x) => {
      // skip closed entries
      if (x.title === 'Closed') return;

      const openingTimes = hoursRegex.exec(x.title);
      if (openingTimes) {
        const openingTime = moment.tz(`${openingTimes[1]}:${openingTimes[2]}`, 'HH:mm', this.config.timezone);
        const closingTime = moment.tz(`${openingTimes[3]}:${openingTimes[4]}`, 'HH:mm', this.config.timezone);

        parkSchedule.push({
          "date": x.date,
          "type": "OPERATING",
          "openingTime": moment.tz(`${x.date} ${openingTime.format('HH:mm')}`, 'YYYY-MM-DD HH:mm', this.config.timezone).format(),
          "closingTime": moment.tz(`${x.date} ${closingTime.format('HH:mm')}`, 'YYYY-MM-DD HH:mm', this.config.timezone).format(),
        });
      }
    });

    // Apply live overwrite for today's closing time from park-infos API
    if (parkInfos) {
      const todayIdx = parkSchedule.findIndex((x) => x.date === today);

      if (todayIdx >= 0) {
        const entry = parkSchedule[todayIdx];
        const apiClose = moment.tz(parkInfos.close, 'YYYY-MM-DD HH:mm:ss', this.config.timezone);
        const calOpen = moment.tz(entry.openingTime, this.config.timezone);

        // Override closing time only if API provides a value for today and it's after the opening time
        if (apiClose.format('YYYY-MM-DD') === today && apiClose.isAfter(calOpen)) {
          entry.closingTime = apiClose.format();
        }
      }
    }

    return [
      {
        _id: 'phantasialand',
        schedule: parkSchedule,
      },
    ];
  }
}
