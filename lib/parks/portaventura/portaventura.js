...
export class PortAventuraWorld extends Destination {
  ...
  async buildEntityLiveData() {
    ...
    Object.keys(liveDataGrouped).map((entry) => {
      ...
      const nonNullEntries = liveDataGrouped[entry].filter((x) => x.time !== null);
      ...
      if (attraction.time === null || attraction.time === undefined) {
        rideData.status = statusType.closed;
      }

      if (attraction.time !== undefined) {
        rideData.queue = {
          [queueType.standBy]: {
            waitTime: attraction.time,
          },
        };
      }
