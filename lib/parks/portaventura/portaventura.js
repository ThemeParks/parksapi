    ...
    return parkData.map((park) => {
      return {
        ...
        schedule: dateData.map((x) => {
          ...
          const match = /(\d{2}):(\d{2}) - (\d{2}):(\d{2})/.exec(dataForThisPark.horario || "");
          if (!match) {
            return null;
          }
          ...
          return {
            ...
            type: "OPERATING",
            openingTime: x.date.clone().hours(parseInt(match[1], 10)).minutes(parseInt(match[2], 10)).seconds(0),
            closingTime: x.date.clone().hours(parseInt(match[3], 10)).minutes(parseInt(match[4], 10)).seconds(0),
          };
        }).filter((x) => !!x),
      };
    });
