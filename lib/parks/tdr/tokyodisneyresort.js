...
export class TokyoDisneyResort extends Destination {
  ...
  constructor(options = {}) {
    ...
    this.http.injectForDomain({
      ...
      if (!options.ignoreDeviceID) {
        ...
        if (!deviceID) throw new Error('Failed to fetch device ID for Tokyo Disney Resort API');
  ...
  async fetchAllFacilitiesData() {
    ...
    const resp = await this.http('GET', `${this.config.apiBase}/rest/v3/facilities`, undefined, {
      headers,
    });
  ...
  async _update() {
    const resp = await this.http(
      'GET',
      `${this.config.apiBase}/rest/v5/facilities/conditions`,
    );
