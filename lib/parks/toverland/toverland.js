...
export class Toverland extends Destination {
  ...
  async _fetchWeekCalendar(month, year) {
    ...
    // create string like: &tx_calendar_ajax%5Bmonth%5D=3&tx_calendar_ajax%5Byear%5D=2024&tx_calendar_ajax%5Baction%5D=month
    const queryString = `tx_calendar_ajax%5Bmonth%5D=${month}&tx_calendar_ajax%5Byear%5D=${year}&tx_calendar_ajax%5Baction%5D=month`

    const resp = await this.http('GET', `${this.config.calendarUrl}&${queryString}`);
    return resp.body;
  }
