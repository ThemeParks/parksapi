...
export class DisneylandParis extends Destination {
  ...
  buildBaseEntityObject(data) {
    ...
    if (data?.coordinates) {
      ...
      if (entrance) {
        ...
        // default to resort location if we have no entrance
        entity.location = {
          longitude: 2.781826,
          latitude: 48.868720,
        };
  ...
  async filterPOIData(filterObj) {
    ...
    const attrs = Object.keys(parkData).reduce((prev, curr) => {
      ...
      // hard-coded new attractions
      // P2EA00: Frozen Ever After
      // P2DA00: Raiponce Tandled Spin
      // P2EA02: Entry to World of Frozen land
      if (x?.id === 'P2EA00' || x?.id === 'P2DA00' || x?.id === 'P2EA02') {
        return true;
      }
      ...
      if (access !== undefined) {
        live.queue[queueType.paidReturnTime] = {
          ...
          price: {
            ...
            // format price using Intl with French locale
    ...
    for (; now.isSameOrBefore(end, 'day'); now.add(1, 'day')) {
      ...
      dateData.forEach((x) => {
        ...
        x.schedules.forEach((hours) => {
          if (hours.status === 'REFURBISHMENT' || hours.status === 'CLOSED') {
            // skip closed times
            return;
          }
          ...
          let typeForSchedule = scheduleType.operating;
          let description = undefined;
          if (hours.status === 'EXTRA_MAGIC_HOURS') {
            typeForSchedule = scheduleType.extraHours;
            description = 'Extra Magic Hours';
          } else if (hours.status === 'PERFORMANCE_TIME') {
            typeForSchedule = scheduleType.informational;
