...
export class DisneylandParis extends Destination {
  ...
  constructor(options = {}) {
    ...
    if (!this.config.standbyApiRefreshToken) throw new Error('Missing Disneyland Paris Refresh Token');
    ...
    this.http.injectForDomain({
      ...
      const authData = await this.getAuthToken();
      if (!authData) {
        throw new Error(`Unable to get auth token for DLP premiere access: ${JSON.stringify(authData)}`);
      }
    ...
    return waits.filter((x) => x.type === 'Attraction').map((time) => {
      ...
      if (rideVQueue !== undefined) {
        live.queue[queueType.returnTime] = {
          returnStart: rideVQueue.timeSlotStartDatetime ? moment.tz(rideVQueue.timeSlotStartDatetime, 'YYYY-MM-DD HH:mm', this.config.timezone) : null,
          returnEnd: rideVQueue.timeSlotEndDatetime ? moment.tz(rideVQueue.timeSlotEndDatetime, 'YYYY-MM-DD HH:mm', this.config.timezone) : null,
      ...
      if (access !== undefined) {
        live.queue[queueType.paidReturnTime] = {
          returnStart: access.nextTimeSlotStartDateTime ? moment.tz(access.nextTimeSlotStartDateTime, this.config.timezone) : null,
          returnEnd: access.nextTimeSlotEndDateTime ? moment.tz(access.nextTimeSlotEndDateTime, this.config.timezone) : null,
