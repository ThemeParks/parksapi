...
async function getResortVirtualQueues() {
  ...
  return await parkDataController.cache.wrapGlobal(`dlp_virtualqueues`, async () => {
    return (await parkDataController.http('GET', parkDataController.config.standbyApiBase)).body;
  }, 1000 * 30); // 30 seconds
}
...
export class DisneylandParisPark extends Park {
  ...
  constructor(options = {}) {
    ...
    options.standbyApiAuth = options.standbyApiAuth || '';
    ...
    if (!this.config.standbyApiAuth) throw new Error('Missing Disneyland Paris standbyApiAuth');
    ...
    this.injectForDomain({
      ...
      options.headers['authorization'] = `BEARER ${this.config.standbyApiAuth}`,
      options.headers.accept = 'application/json, text/plain, */*';
    });
  }
  ...
  async _update() {
    ...
    let vQData = await getResortVirtualQueues();
    if (vQData === undefined) {
      vQData = [];
    }
    ...
    await Promise.allSettled(waitTimes.map(async (time) => {
      ...
      // TODO - handle virtual queues (valid October 6th)
    }));
  }
