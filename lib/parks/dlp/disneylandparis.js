...
export class DisneylandParisPark extends Park {
  ...
  async _buildAttractionObject(attractionID) {
    ...
    if (attr.height !== undefined) {
      attr.height.forEach((height) => {
        ...
        const heightVal = /([\d\.]+)/.exec(height.value);
        if (heightVal) {
          tags.push({
            type: tagType.minimumHeight,
            key: height.id,
            value: {
              height: Number(heightVal[1]) * 100,
              unit: 'cm',
            },
          });
        }
  ...
  async _getOperatingHoursForDate(date) {
    ...
    return parkDate.schedules.map((hours) => {
      return {
        openingTime: moment(`${dateString}T${hours.startTime}`, momentParseFormat).tz(this.config.timezone).format(),
        closingTime: moment(`${dateString}T${hours.endTime}`, momentParseFormat).tz(this.config.timezone).format(),
