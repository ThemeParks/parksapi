...
export const HTTP = (function() {
  ...
  const mainFunction = async (method, url, data, options = {}) => {
    ...
    // Use connection pooling with keep-alive (unless agent is explicitly provided)
    if (!options.agent) {
      const urlObj = new URL(url);
      options.agent = urlObj.protocol === 'https:' ? httpsAgent : httpAgent;
    }

    // 10 seconds default timeout for opening socket connection
    options.open_timeout = options.open_timeout !== undefined ? options.open_timeout : 10000;
    ...
    // Limit number of redirects to follow
    options.follow_max = options.follow_max !== undefined ? options.follow_max : 5;
    ...
    // wrap our needle call in a retry with exponential backoff
    return await promiseRetry({
      retries: options.retries === undefined ? 3 : options.retries,
      factor: 2,           // Exponential backoff factor
      minTimeout: 1000,    // Start with 1s delay between retries
      maxTimeout: 5000,    // Maximum 5s delay between retries
    }, async (retryFn) => {
      ...
      return httpFunc(method, url, data, options).then(async (resp) => {
        ...
        // Don't intercept promise-retry internal errors - let them propagate for retry logic
        ...
        const enhancedError = new Error(
          `HTTP ${method} request failed: ${err.message}\n` +
          `URL: ${url}\n` +
        ...
        console.error(`[HTTP Error] Code: ${err.code || 'UNKNOWN'}`);
